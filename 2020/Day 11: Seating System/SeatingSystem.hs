...
rule2Seat seats y x seat
  ...
  where
    ...
    -- Turns out that doesn't work, checking all the seats that are affected by
    -- a seat is more work and slows down the calculation. It's simpler and
    -- faster to just check for every seat whether it needs updating.
